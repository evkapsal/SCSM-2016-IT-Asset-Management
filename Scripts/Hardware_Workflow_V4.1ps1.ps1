...
if ($HardwareConnectorAdminSettingObj.IsActive -eq $true)
{
		...
		foreach ($WindowsComputer in $WindowsComputers)
		{
			...
			else
			{
				...
				if ($ObjLast -ne $WinCLast)
				{
					...
					$emg = New-Object Microsoft.EnterpriseManagement.EnterpriseManagementGroup "CORPSCSM01"
	...
	if ($NetworkDevices)
	{
		foreach ($NetDevice in $NetworkDevices)
		{
			...
			{
				if ($($NetDevExists.LastModifiedSync) -lt $($NetDevice.LastModified))
				{
					$ObjHistory = $null
					$emg = $null
					$emg = New-Object Microsoft.EnterpriseManagement.EnterpriseManagementGroup "CORPSCSM01"
